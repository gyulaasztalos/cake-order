...
# ---------------------------------------------------------------------------
# cake-order — multi-arch (linux/amd64 + linux/arm64 for rPi5) image.
# Build with uv; run as non-root. See PLANNING.md §4 (kept out of this repo).
# ---------------------------------------------------------------------------

# ---- builder: resolve & install deps into a venv with uv --------------------
FROM ghcr.io/astral-sh/uv:0.11.31-python3.14-trixie-slim AS builder
